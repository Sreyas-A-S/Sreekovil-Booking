@forelse($hotels as $hotel)
    <tr class="hover:bg-slate-50/50 transition-colors">
        <td class="py-4 px-6 text-sm font-mono text-saffron-600/50 whitespace-nowrap">#{{ $hotel->id }}</td>
        <td class="py-4 px-6">
        ...
        <td class="py-4 px-6 text-slate-500">
            <div class="flex items-center gap-1.5 text-sm whitespace-nowrap">
                ...
                {{ $hotel->temple->name ?? 'N/A' }}
            </div>
        </td>
        <td class="py-4 px-6 text-slate-500 whitespace-nowrap">
            <div class="font-medium text-slate-900">₹{{ number_format($hotel->onwards_price, 2) }}</div>
        </td>
    ...
    <tr>
        <td colspan="5" class="py-12 text-center text-slate-500">
            <div class="flex flex-col items-center gap-2">
                ...
                <p>No hotels found. Start by adding one!</p>
            </div>
        </td>
    </tr>
@endforelse